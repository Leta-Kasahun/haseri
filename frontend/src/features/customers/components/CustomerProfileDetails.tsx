...
export function CustomerProfileDetails({ onSave, isEmbedded = false }: CustomerProfileDetailsProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      setFormData({
        ...
        label: (user as any)?.address?.label || "Home",
      });
    }
  }, [user]);
  ...
  return (
    <div className={`${isEmbedded ? "p-10" : "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-none p-10 shadow-xl"} relative transition-all duration-300`}>
      
      {/* Edit Trigger */}
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <button className="absolute top-8 right-10 text-slate-400 hover:text-primary transition-colors p-2 active:scale-95">
        ...
        <DialogContent className="sm:max-w-[600px] rounded-none border-4 border-slate-900 p-8 max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black uppercase tracking-tighter italic">
              Edit <span className="text-primary">Profile</span> Console
            </DialogTitle>
          </DialogHeader>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 py-8">
            <div className="space-y-4 md:col-span-2">
              <p className="text-[9px] font-black uppercase tracking-[0.4em] text-primary mb-2 border-b border-primary/20 pb-2">Identity Configuration</p>
            ...
            <div className="space-y-4 md:col-span-2 pt-4">
              <p className="text-[9px] font-black uppercase tracking-[0.4em] text-primary mb-2 border-b border-primary/20 pb-2">Communications</p>
            ...
            <div className="space-y-4 md:col-span-2 pt-4">
              <p className="text-[9px] font-black uppercase tracking-[0.4em] text-primary mb-2 border-b border-primary/20 pb-2">Service Location</p>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Address Label</label>
                  <Input name="label" value={formData.label} onChange={handleChange} className="h-12 rounded-none border-2 border-slate-200 focus:border-slate-900 font-bold" />
                </div>
              </div>
              <div className="space-y-2 pt-2">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Specific Landmarks</label>
              ...
              {group.fields.map((field, fIdx) => (
                <div key={fIdx} className="flex items-baseline gap-2">
                  ...
                  <span className="text-[13px] font-black uppercase tracking-tight text-slate-900 dark:text-white truncate">
                    {field.value || <span className="text-slate-200 italic">Unconfigured</span>}
                  </span>
                </div>
              ))}
            </div>
